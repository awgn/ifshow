...
static std::string 
show_basic_mii(int sock, int phy_id)
{
    char buf[128]=  {'\0'};
    unsigned int i;
    ...
    for (i = 0; i < (MII_BASIC_MAX+1); i++)
        mii_val[i] = mdio_read(sock, i);

    if (mii_val[MII_BMCR] == 0xffff) {
        // fprintf(stderr, "  No MII transceiver present!.\n");
        // return -1;
        return std::string(" no MII transceiver present!");
    }
...
static inline int ifr_prep(int skfd, const char *ifname, int maybe)
{
    struct mii_data *mii = (struct mii_data *)&ifr.ifr_data;
...
// int do_one_xcvr(int skfd, const char *ifname, int maybe)
std::string 
do_one_xcvr(int skfd, const char *ifname, int maybe)
{
    struct mii_data *mii = (struct mii_data *)&ifr.ifr_data;
